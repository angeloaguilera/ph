// app/api/administration/condominium-management/service.ts
import { genId, readData, writeData, validateChecklist, DataKind, removeUploadedFileByUrl } from "./_lib";

const KIND: DataKind = "service";

export async function readServices(): Promise<any[]> {
  return readData(KIND);
}

export async function writeServices(arr: any[]) {
  return writeData(KIND, arr);
}

export async function upsertService(item: any) {
